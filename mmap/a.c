...
static int my_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	vma->vm_pgoff = virt_to_phys(my_data) >> PAGE_SHIFT; // 把kernel VA轉PA，在用offset轉Physical Page number

	status = remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff, 
				 vma->vm_end - vma->vm_start, vma->vm_page_prot); //把這個PFN映射到user的VMA的[vm_start,vm_end]範圍，讓user的那段VA，直接指向此page
	if(status) {
